%Practicing generating animation of orbital trajectory
...
figure(5)
   ...
   plotVect=[1:(animationTimestep/dt):N_last_inc+animationTimestep];
     ...
     for j=1:length(plotVect)
         ...
         earthPlot
         hold on
         plot3(insat_position(1,1:N_last_inc),insat_position(2,1:N_last_inc),insat_position(3,1:N_last_inc),'r',LineWidth=1.25);
         ...
         if j == 1 
              imwrite(imind,cm,'animation_orbit.gif','gif', 'Loopcount',1); 
         else 
              imwrite(imind,cm,'animation_orbit.gif','gif','WriteMode','append'); 
         end 
      end
